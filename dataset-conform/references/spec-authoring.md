...
4. **找出跨字段约束**。这类规则最容易被忽略，也最容易出错：索引指向另一个数组（`correctAnswer` → `options`）、子字段必须取自父字段的某个变体、数组内元素互不重复。
5. **把说不清的规则写进 `semanticRules`**，不要硬塞进机械校验。判断标准：这条规则能否只靠"数一数、比一比、匹配正则"得出结论？不能，就是语义规则。
6. **校准**：对参考样例跑 `audit`，要求 0 error。有 error 就放宽约束或降级为 `warn`，并在 `spec.md` 记录理由。
